...
此时 storyboard 每个 active 镜头必须包含 `rhythm_role`、`state_dependencies`、非空对象 `state_before`/`state_after`、非空字符串 `composition_16x9`、`recomposition_9x16`、`platform_capability_needs`，以及电影化导演声明 `coverage_role`、`kinetic_profile`；除最后一镜外，前一镜还必须声明通往下一镜的 `transition_contract`。`state_dependencies` 必须按有效数字 `sequence` 形成只指向更小序号镜头的有向无环图；不得用数组位置推断上游，active 镜头序号不得重复。本镜 `state_before` 必须包含每个依赖镜头 `state_after` 的全部字段并保持对应值相同，可额外包含其他已声明依赖交接的状态。`recomposition_9x16.composition` 是非空字符串，`safe_areas` 是非空数组且每项为非空字符串。
...
`quality_report.checks.narrative_clarity` 必须逐项记录 `protagonist`、`goal`、`obstacle`、`causality`、`ending_change` 的 `pass` 或 `fail`。`quality_report.checks.continuity_integrity` 必须记录 `status` 与 `unresolved_conflicts`。电影化包还必须记录 `quality_report.checks.intent_fidelity`、`quality_report.checks.director_quality` 与 `quality_report.checks.inner_life_audit`，每项都包含 `status`、`unresolved_conflicts` 与非空 `evidence_refs`。prompt 的 `approval_status` 为 `draft`、`blocked` 或 `final`；job 的 `approval_status` 为 `blocked`、`non_executable` 或 `approved`。任一硬门失败时，`quality_report.ready` 只能是 `false`，prompt 只能为 `draft`/`blocked`，job 只能为 `blocked`/`non_executable`；硬门通过但 `ready: false` 时也保持这组非最终状态。只有 `ready: true` 且叙事、连续性、意图、导演质量、内心线与身份硬门都通过时，prompt 才能为 `final`、job 才能为 `approved`。
...
电影化 active 镜头另有三个真实 JSON 字段。`coverage_role` is a non-empty, deduplicated string array，元素只能是 `setup`、`anticipation`、`action`、`impact`、`reaction`、`consequence`、`transition`、`aftermath`；全部 active 镜头的 coverage union 至少包含 `action`、`reaction`、`consequence`，允许同镜多角色，不要求凑齐八种。`kinetic_profile` 精确声明 `subject_motion`、`performance_change`、`camera_motion`、`environment_motion`、`motion_layers_required`、`intentional_hold`、`hold_reason`、`acceptance_evidence`; `acceptance_evidence` is a non-empty string. 非 hold 镜头至少声明两层且实际填写相应运动描述；hold 仅允许 0/1 层，并要求非空原因和验收证据。每个相邻边界由前一镜的 `transition_contract` 声明 `type`、`visual_precondition`、`incoming_match`、非负整数 `duration_frames`、`audio_bridge_cue_id`、`story_reason`、`fallback`；最后一镜允许该字段缺失或为 null。这里仅声明导演意图，不包含剪辑层字段。
...
For cinematic finished-film delivery, `edit_master_plan` adds one optional `cinematic_validation`. It has exactly 17 required fields aligned with `scripts/cinematic_validation.py`: `declared_mode`, `genre`, `content_consistency`, `intent_fidelity`, `character_identity_integrity`, `action_reaction_coverage`, `kinetic_profile_audit`, `shot_scale_and_composition_variety`, `transition_fulfillment`, `audio_presence_and_structure`, `static_hold_audit`, `source_motion_review`, `director_quality`, `subtext_fidelity`, `ppt_risk_flags`, `evidence_refs`, and `cinematic_ready`.

Run `scripts/validate_edit_plan.py ... --require-cinematic` only for explicit cinematic/movie/blockbuster/大片/电影感 intent or an existing cinematic declaration. Ordinary finished output uses the existing finish gates. All twelve audit objects must have `status: passed`; `intent_fidelity` maps every declared intent and every actual edit unit through resolving edit-unit `intent_refs`; `director_quality` reviews every actual edit unit and requires empty `rejected_pattern_flags`; `subtext_fidelity` reviews every actual edit unit, requires non-empty `inner_life_refs` and `evidence_refs`, and requires empty `unresolved_conflicts`; action coverage requires `action`, `reaction`, and `consequence`; kinetic evidence covers every actual edit unit; transition evidence covers every actual adjacent pair and carries non-empty `incoming_match` plus non-negative integer `duration_frames`; missing audio requires `silent_form_authorization` and passed visual audits; `ppt_risk_flags` must be empty before ready.

`cinematic_ready` is linked to, but distinct from, technical render state. Passed plan audits may leave `cinematic_ready: false`; they do not prove actual-film acceptance. `cinematic_ready: true` requires `plan_status: rendered`, every requested delivery rendered and ready, `edit_validation.ready: true`, valid tool/probe/rendered-output evidence, and `source_motion_review` actual-output fields `actual_output_review_status: passed`, exact `reviewed_delivery_ids`, non-empty `frame_change_evidence_refs`, and non-empty `contact_sheet_evidence_refs`. The two ref arrays are IDs into optional `execution.review_evidence`, not free-form strings; each strict record binds one requested delivery and evidence type to a passed artifact and verified tool. Each type must cover every requested delivery exactly. Cinematic blockers force blocked aggregate/delivery/edit readiness; technical render evidence never overrides a creative blocker.
