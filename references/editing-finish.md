...
Each timeline records `timeline_id`, `aspect_ratio`, `resolution`, `frame_rate`, `duration_seconds`, `video_tracks`, `audio_track_refs`, `text_track_refs`, and `export_refs`. Every edit unit records `edit_unit_id`, `sequence`, `shot_id`, `asset_id`, `timeline_in_seconds`, `timeline_out_seconds`, `duration_seconds`, `source_in_seconds`, `source_out_seconds`, `story_function`, `cut_reason`, `transition_in`, `transition_out`, `speed`, `freeze_frames`, `stabilization`, `reframe`, `safe_area`, `scale`, `position`, `opening_state`, `closing_state`, `continuity_ids`, `locked_event_ids`, `audio_cue_ids`, `text_cue_ids`, `look_instruction`, `risk_triggers`, and `approval_status`. For cinematic delivery, edit-unit `intent_refs` is required on every actual V1 first-release edit unit and must resolve to `cinematic_validation.intent_fidelity.intent_ids`; edit-unit dialogue, silence, reaction, and emotional consequence must also be reviewed by `cinematic_validation.subtext_fidelity`.
...
For cinematic delivery, verify `transition_fulfillment` against every actual adjacent edit-unit pair. Each boundary carries its declared type, story reason, visual precondition, non-empty `incoming_match`, non-negative integer `duration_frames`, audio bridge cue or explicit none, fallback, editing-only fulfillment status, and evidence. A supported hard cut may pass when motivated and evidenced; if an adapter drops a storyboard transition, declare and review its fallback and create a new dry-run rather than silently turning every boundary into a hard cut.
...
The optional `cinematic_validation` object is the creative-finish authority for movie/cinematic delivery. Validate it with `--require-cinematic`; its audits cover content consistency, `intent_fidelity`, character identity, action/reaction/consequence coverage, kinetic profiles, shot/composition variety, `transition_fulfillment`, `audio_presence_and_structure`, intentional holds, source motion, `director_quality`, `subtext_fidelity`, and `ppt_risk_flags`.

### Plan acceptance

Plan acceptance validates the declared Canon before it is called executable or ready for rendering. It checks timeline structure and duration math; edit-unit `intent_refs`; `intent_fidelity` mappings; `director_quality` reviewed edit units and rejected pattern flags; `subtext_fidelity` reviewed edit units, inner-life refs, evidence refs, and empty unresolved conflicts; action/reaction/consequence `coverage`; legal source bounds and `cut points`; declared `transitions`, visual preconditions, incoming matches, durations, audio bridges, and fallbacks; cleared `audio` structure and routing; and `prohibited items` such as placeholders in final, unresolved drift, unsupported dependencies, or PPT-risk patterns. The state ?plan audits passed? proves only that the timeline and acceptance targets are coherent; it does not set `cinematic_ready: true` or prove that an output file contains the intended action, performance, cut, transition, subtext, inner life, or sound.

### Actual-film acceptance

Actual-film acceptance starts only at `plan_status: rendered`, after every requested delivery is rendered and ready, `edit_validation.ready` is true, and valid tool/probe/rendered-output evidence exists. Use `ffprobe` for container `streams`, codecs, dimensions, frame rate, channels, and `duration`; use an `FFmpeg or equivalent output probe` to sample boundary windows and verify each declared `cut/frame change`. Build an extracted-frame `contact sheet` and conduct `visual review` against the Canon to confirm `character action`, `performance change`, identity, repeated poses, source motion, composition, and `actual transitions`. In `source_motion_review`, record `actual_output_review_status` as `passed`, `reviewed_delivery_ids` exactly matching requested deliveries, non-empty `frame_change_evidence_refs`, and non-empty `contact_sheet_evidence_refs`. These refs are IDs into optional `execution.review_evidence`, not free-form strings. Each strict evidence record names one requested `delivery_id`, a `frame_change` or `contact_sheet` type, a non-empty artifact, passed status, and a verified `tool_evidence_ref`; each type must cover each requested delivery exactly. Record tool/version, commands or review method, artifact references, and per-check results; never imply that merely planning or rendering performed these checks.
...
`rendered` does not equal cinematic readiness. `rendered` means the requested outputs have the required tool/probe evidence; `cinematic_ready: true` additionally means every cinematic audit passed, `intent_fidelity` resolves every actual edit unit, `director_quality` has empty `rejected_pattern_flags`, `subtext_fidelity` covers every actual edit unit with no unresolved inner-life conflicts, `ppt_risk_flags` is empty, and the actual-output review fields above are valid. Dry-run, authorized, draft, or merely plan-audited states keep `cinematic_ready: false`. If cinematic validation fails, aggregate status, delivery readiness, and edit validation remain blocked even when a technical file exists. Static poster posing, subject-less particles/background motion, unrelated spectacle, invisible psychology, mechanical shot/reverse-shot coverage, content mismatch, identity/model-lock drift, or unfulfilled transitions directly fail the cinematic master.
...
When `software_targets` contains `ai_editor`, compilation writes `ai_editor_plan.json` as a derived AI-editor handoff. The AI editor consumes that read-only JSON rather than editing the Canon directly; `edit_master_plan.json` remains the sole edit Canon, and proposed changes return to the Canon before recompilation. The derivative carries the source edit-plan ID, media bindings, timelines and edit units, time and transition data, audio and text tracks, look and delivery/export specifications, ordered execution steps, authorization references, and edit/cinematic/tool/probe/review evidence references. It executes dependencies in order: probe, bind, build timelines, apply text, build audio, apply look, export, then validate. Jianying/CapCut, Premiere Pro, DaVinci Resolve, FFmpeg, current NLEs, and future AI editors all consume derivatives of the same Canon and QC evidence; adapters may expose only verified public formats or a `manual_or_unverified` handoff and must not invent a private schema. Each step records `pending`, `running`, `passed`, `failed`, or `blocked`, including tool/version evidence and artifacts. Compilation produces this AI-ready plan but does not grant execution. A failed AI step returns the earliest repairable Canon object rather than patching only the rendered output.
