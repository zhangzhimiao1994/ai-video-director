...
`rendered` does not equal cinematic readiness. `rendered` means the requested outputs have the required tool/probe evidence; `cinematic_ready: true` additionally means every cinematic audit passed, `intent_fidelity` resolves every actual edit unit, `director_quality` has empty `rejected_pattern_flags`, `ppt_risk_flags` is empty, and the actual-output review fields above are valid. Dry-run, authorized, draft, or merely plan-audited states keep `cinematic_ready: false`. If cinematic validation fails, aggregate status, delivery readiness, and edit validation remain blocked even when a technical file exists. Static poster posing, subject-less particles/background motion, unrelated spectacle, mechanical shot/reverse-shot coverage, content mismatch, identity/model-lock drift, or unfulfilled transitions directly fail the cinematic master.
