...
When cinematic mode is active, every `shot_prompts[]` record must carry `intent_refs` that exactly match the bound storyboard shot. The prompt may translate the shot into model-friendly language, but it must not add a new event, motivation, reveal, power relation, identity detail, or ending. `global_lock_block` preserves character/model/prop/location/look identity; `shot_direction_block` carries the director choice and camera necessity; `platform_compile_block` only maps verified platform capability and unresolved manual configuration.

If a provider-specific prompt wants a spectacular image that cannot point back to `intent_refs`, keep it out of the final prompt and record the rejected choice in the upstream scene or shot director evidence.
## Inner-life prompt addendum

When compiling cinematic prompts, inherit `inner_state_ref`, `visible_psychological_evidence`, `subtext_action`, `emotional_leak`, and `dialogue_or_silence_function` from the storyboard. Put only observable evidence into `universal_prompt_en`: breath, blink, gaze, hand pressure, hesitation, concealment, recoil, reaction, motivated silence, or sound-space change. Do not translate inner life into abstract emotion adjectives alone; if the prompt says a character feels something, it must also state the visible behavior that proves it.
